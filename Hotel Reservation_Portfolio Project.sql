...
-- Data aggregation from differnce years
...
-- Create a new table with all of data that we need
...
-- Now we have a full data table and it is used for all of querries behind
-- 1. Look at the revenue and revenue percentage changes year over year

 WITH TEMP AS (
	SELECT hotel,
		arrival_date_year,
		ROUND(SUM((stays_in_weekend_nights + stays_in_week_nights) * adr * (1 -  Discount) + meal_cost), 2) AS revenue
	FROM FULL_DATA
...
SELECT a.hotel,
	a.arrival_date_year,
	a.revenue,
	b.arrival_date_year AS previous_year,
	CASE 
		WHEN b.arrival_date_year IS NOT NULL 
		THEN CONCAT(ROUND(((a.revenue - b.revenue)/b.revenue) * 100, 2), '%') 
...
-- 3. Look at the average of approximately bookings per day

SELECT *,
	total_bookings /
	CASE 
		WHEN (cast(arrival_date_year as int) % 4 = 0 AND cast(arrival_date_year as int) % 100 <> 0) OR cast(arrival_date_year as int) % 400 = 0 THEN 366
		ELSE 365 END AS average_bookings_per_day
FROM (
	SELECT hotel,
		arrival_date_year,	
...
-- 9. Look at the most the market segment of booking
...
-- 10. Look at the most distribution channel of booking
